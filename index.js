...
const { colorPalette } = require("../config/colorPalette.json");
...
const httpServer = createServer({
  key: readFileSync("/etc/letsencrypt/live/www.zlyfer.net/privkey.pem"),
  cert: readFileSync("/etc/letsencrypt/live/www.zlyfer.net/cert.pem"),
  // ca: readFileSync("/etc/letsencrypt/live/zlyfer.net/chain.pem"),
  // rejectUnauthorized: false
});
io = new Server(httpServer, {
  cors: {
    origin: ["https://zlyfer.net", "https://old.zlyfer.net", "https://artistio.zlyfer.net"],
    methods: ["GET", "POST"],
  },
});
// END
  ...
  socket.on("send_message", (data) => {
    if (
      ...
      let score = Math.floor(
        rooms[data.author.room].timer / (rooms[data.author.room].maxTimer / 100) - rooms[data.author.room].guessedIt.length
      );
    ...
    } else if (rooms[data.author.room].guessedIt.includes(data.author.id) || rooms[data.author.room].artist == data.author.id) {
      let newMSG = {
        author: data.author,
...
function checkRoundEnd(roomname) {
  if (rooms[roomname].guessedIt.length >= Object.keys(rooms[roomname].players).length - 1 && rooms[roomname].gamestate != "Lobby") {
    if (rooms[roomname].artistScore) {
      rooms[roomname].scores[rooms[roomname].artist] =
        rooms[roomname].guessedIt.length * (100 / (Object.keys(rooms[roomname].players).length - 1));
    }
